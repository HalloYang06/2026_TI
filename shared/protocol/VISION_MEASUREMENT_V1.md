...
目标输入为`100 Hz`，EdgeTalk接收能力仍按不低于`240 Hz`验收，为真实相机保留调度余量。
M55估计与控制以`200 Hz`运行：有新视觉序号时更新观测，无新帧时只做模型预测，
禁止重复融合旧测量。
...
当前C++ OpenCV服务在`camera.read()`返回后读取主机`steady_clock`，因此它提供的是
“帧出队/交付时刻”，还不是曝光中点。该值可用于排序和测量处理耗时，但在接入V4L2
缓冲时间戳并完成树莓派—M33时钟同步前，M55不得把它作为真正OOSM采集时刻。
...
5. 连续约3帧缺失（约30 ms）开始膨胀观测协方差；`50 ms`进入降级，`100 ms`判定视觉失锁。最终阈值需按实测P99延迟调整。

## 带宽与频率

| 帧率 | 纯协议负载 | 结论 |
|---:|---:|---|
| 100 Hz | 6,400 B/s | 正式目标，周期10 ms |
...
当前真实相机桥接尚未取得曝光时间、曝光稳定状态或圆度结果，因此不会置
`EXPOSURE_STABLE/CONTOUR_ROUND`；现有`confidence=0.85`仍是临时常数，只可用于shadow
联调。正式闭环前必须由C++检测器输出可回放验证的逐帧置信度，不能继续使用常数。
