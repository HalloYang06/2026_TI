...
- 接近机构死点时的有效性检查；
- MSPM0 500 Hz IMU采集、PSoC 500 Hz电机任务、PSoC 200 Hz外环和100 Hz视觉的3 kHz公共仿真网格；
...
- `sfun_vehicle_imu.m`：MSPM0 500 Hz IMU采集、噪声、偏置和处理延迟。
...
| 测试 | 结果 |
|---|---|
| 正常车辆，100 Hz，初始误差5 mm | RMS 1.750 mm，峰值5.000 mm，严格1 cm通过 |
| 起步/制动/坑洼，100 Hz，初始误差5 mm | RMS 2.721 mm，峰值6.821 mm，严格1 cm通过 |
| 最坏组合恢复，优化方案，初始误差25 mm | RMS 10.48 mm，未掉球，机械安全通过 |
| 最坏组合恢复，仅KF-LQI | RMS 13.17 mm，未掉球，机械安全通过 |
| 3.0 m/s²轴向超限脉冲 | 峰值71.99 mm，机械安全失败，作为物理包线边界 |

前两项才是当前脚本中的严格`+-10 mm`测试。压力恢复测试从误差带外开始，不能把
...
- 水管合页/中心轴高度 `82 mm`，建议做成约 `±3 mm` 竖向可调；
- 水管水平时，蓝杆上端铰点 `B` 在水平方向距电机轴心 `O` 约 `18 mm`；
- 支撑台高度满足“不小于 `50 mm`”的约束；
- 如果约 50 mm 外径的水管由台面托住，水管底面约为 `57 mm` 高。
...
bp.actuator.max_rate = 1.2;    % rad/s，水管端指令斜率
```

初次实物测试不要直接开放 14 N·m 峰值。先把水管固定可靠、钢球取下，在 1°、2°、3° 小阶跃下记录：

实机第一次测试建议暂时把 `user_speed_limit` 降到 `3 rad/s`；确认连杆无干涉、
RS00 电流和温度正常后，再根据钢球闭环需要逐步提高。示例机构在 3 rad/s 限速下
只能保证最不利位置约 `0.44 rad/s` 的水管速度，仿真会出现极限环并掉球。
