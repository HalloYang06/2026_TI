...
- 接近机构死点时的有效性检查；
- 当前WIT约30 Hz唯一IMU样本、PSoC 200 Hz外环、500 Hz电机安全任务和100 Hz视觉的3 kHz公共仿真网格；
...
- `sfun_vehicle_imu.m`：车身IMU约30 Hz唯一源样本、保持输出、噪声、偏置和处理延迟。
...
当前WIT UART为9600 bit/s。每个完整样本组包含3个11字节8N1帧，至少330 bit，因此
理论上限约29.1组/s。改为115200 bit/s后，串口理论上限约349.1组/s：

- 100 Hz完整组约占28.6%链路；
- 200 Hz完整组约占57.3%，是当前推荐升级目标；
- 500 Hz完整组需要约165 kbit/s，115200仍然不够。

必须同时修改WIT模块和MSPM0 UART波特率，并确认WIT输出率寄存器确实设为100或200 Hz。
控制器按源序号判断新样本，200 Hz CAN重复镜像不能冒充新IMU数据。可用下面的what-if仿真
比较115200/200 Hz升级效果，而不改变默认30 Hz实测基线：
...
| 正常车辆，100 Hz视觉，初始误差5 mm | RMS 4.199 mm，峰值8.701 mm，严格1 cm通过 |
| 起步/制动/坑洼，100 Hz视觉，初始误差5 mm | RMS 48.173 mm，峰值101.288 mm，严格1 cm失败 |
| 可行组合恢复，优化方案，初始误差25 mm | RMS 17.78 mm，峰值45.73 mm，机械安全通过 |
| 可行组合恢复，仅KF-LQI | RMS 33.26 mm，峰值88.04 mm，机械安全通过 |
| 同一可行组合，115200/200 Hz IMU what-if | RMS 10.62 mm，峰值25.00 mm，机械安全通过 |
...
bp.contact.rolling_resistance = 0.002;
```
...
- 钢球可滚动的水管有效长度 `250 mm`；
- 水管合页到蓝杆连接端 `CB = 300.1 mm`，与250 mm有效滚动长度不是同一个量；
- RS00 主动杆 `OA = 35.0 mm`；
- 蓝色连接杆 `AB = 55.5 mm`；
- RS00 输出轴高度 `38 mm`。
- 水管右侧合页轴 `C` 高度 `93 mm`；
...
- 钢球直径和质量；
- 水管内径；
- 250 mm有效滚动段相对C轴的位置，即钢球中心零点到C轴的真实距离；
