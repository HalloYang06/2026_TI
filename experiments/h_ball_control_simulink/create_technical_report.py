...
def create_flow_image():
    ...
    box((55, 390, 375, 575), "MSPM0", ["车身IMU源样本", "200 Hz + 时间戳"], LIGHT_BLUE)
...
def build_report():
    ...
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    r = p.add_run("版本：V1.4（实测连杆、实机视觉与115200/200 Hz IMU基线）")
    ...
    add_table(
        ...
        ["项目", "当前设计基线", "性质"],
        [
            ("水管物理长度 / CB", "250 mm / 300.1 mm", "用户给定；两者不是同一个量"),
            ("视觉零点到物理挡边", "左右各112 mm", "挡边间距224 mm；球心极限扣半径"),
            ...
            ("IMU唯一源样本率", "200 Hz", "WIT 115200 bit/s；源时间戳待实测"),
            ("外环控制频率", "200 Hz", "设计值"),
            ("视觉频率", "100 Hz", "用户最新确认"),
    ...
    add_table(
        ...
        [
            ...
            ("车体俯仰/轴向加速度", "MSPM0 IMU", "200 Hz唯一源样本；默认8 ms", "保持输入、车辆前馈"),
    ...
    add_callout(
        doc,
        "115200波特率基线",
        "三类11字节8N1帧每个完整组至少330 bit。115200 bit/s理论上限约349.1组/s，"
        "可支持200 Hz完整组，但仍不能支持500 Hz。当前模型使用200 Hz唯一样本和8 ms延迟；"
        "MSPM0源序号与时间戳仍须确认WIT模块的实际输出率。",
        fill=GREEN,
        color="2F6B3A",
    )
    ...
    add_body_paragraph(
        ...
        "固定步长为1/3000 s，使200 Hz IMU源样本、200 Hz控制和100 Hz视觉落在公共仿真网格上。"
    )
    add_figure(
        doc,
    ...
    add_table(
        ...
        [
            ...
            ("IMU", "200 Hz唯一源样本、保持输出、偏置、噪声和处理延迟", "sfun_vehicle_imu.m"),
    ...
    stress_rows = [
        ("低摩擦静止", "7.127", "25.003", "0.608", "0.403", "是", "否"),
        ("正常车辆运动", "6.947", "25.000", "0.742", "0.523", "是", "否"),
        ("启动与急刹", "14.209", "30.214", "1.896", "0.542", "是", "否"),
        ("坑洼+视觉退化", "8.148", "25.000", "1.031", "0.532", "是", "否"),
        ("可行组合（优化）", "10.618", "25.000", "4.338", "0.516", "是", "否"),
        ("可行组合（仅KF-LQI）", "13.462", "26.010", "19.943", "0.700", "是", "否"),
        ("超新机构包线组合", "44.107", "94.311", "148.278", "0.736", "否", "否"),
        ("3.0 m/s²超限脉冲", "80.457", "115.003", "100.310", "0.695", "否", "否"),
    ]
    add_table(
        doc,
        ["场景", "RMS/mm", "峰值/mm", "滑移\nmm/s", "力矩\nN·m", "机械安全", "严格1 cm"],
    ...
    add_body_paragraph(
        doc,
        "在相同可行组合扰动下，加入车辆前馈和扰动估计后，RMS误差相对仅KF-LQI方案降低约21.1%，"
        "峰值滑移也由19.943降至4.338 mm/s。当前115200 bit/s、200 Hz唯一IMU基线结果为"
        "RMS 10.62 mm、峰值25.00 mm，但源频率仍需硬件时间戳验证。3.0 m/s²脉冲产生"
        "115.003 mm峰值和100.310 mm/s滑移，说明"
        "该工况超出推荐物理包线。"
    )
    ...
    add_figure(
        ...
        "图6  相同200 Hz控制器和200 Hz唯一IMU源样本条件下，100、60与30 Hz视觉的位置误差对照。"
    ...
    add_table(
        ...
        [
            ("正常车辆运动", "100 Hz", "2.344", "5.216", "100%", "是"),
            ("正常车辆运动", "60 Hz", "2.431", "5.000", "100%", "是"),
            ("正常车辆运动", "30 Hz", "2.638", "5.000", "100%", "是"),
            ("启动、刹车与坑洼", "100 Hz", "21.453", "60.559", "62.43%", "否"),
            ("启动、刹车与坑洼", "60 Hz", "16.169", "38.746", "54.50%", "否"),
            ("启动、刹车与坑洼", "30 Hz", "43.636", "94.839", "35.40%", "否"),
        ],
    ...
    add_callout(
        doc,
        "结论",
        "新机构和200 Hz IMU源样本下，三种视觉帧率都能通过正常车辆场景，但启停/刹车/坑洼场景"
        "全部失败；帧率从30提高到100 Hz没有消除物理限角和机构限速造成的包线限制。"
        "100 Hz仍是推荐视觉基线，但优化优先级应放在车辆加速度规划、IMU源频率实测和"
    ...
    params = [
        ...
        ("控制/IMU唯一源样本/视觉", "200 / 200 / 100 Hz", "当前模型"),
        ("视觉延迟", "25 ms处理 + 10 ms串口 = 35 ms", "模型假设"),
        ("IMU延迟", "8 ms", "115200串行组+处理/调度近似"),
