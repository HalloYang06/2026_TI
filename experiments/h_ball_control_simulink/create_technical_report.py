...
def create_flow_image():
    ...
    box((55, 390, 375, 575), "MSPM0", ["IMU采样", "500 Hz + 时间戳"], LIGHT_BLUE)
    ...
    box((1215, 145, 1715, 350), "RS00执行器", ["CAN运动模式", "主动杆51.5 mm"], GREEN)
    box((1215, 455, 1715, 660), "两连杆与水管", ["蓝杆65.5 mm", "水管摇杆250 mm"], GREEN)
...
def build_report():
    ...
    add_callout(
        doc,
        "适用方案",
        "RS00关节电机直接驱动51.5 mm主动杆，经65.5 mm连杆带动250 mm水管摇杆；"
        "PSoC Edge E84直接控制RS00，MSPM0以500 Hz采集IMU，树莓派提供视觉位置。",
        fill=LIGHT_BLUE,
    )
    ...
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    r = p.add_run("版本：V1.1（100 Hz仿真基线）")
    set_run_font(r, size=11, color=MUTED)
    p = doc.add_paragraph()
    p.paragraph_format.space_after = Pt(5)
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    r = p.add_run("日期：2026年7月30日")
    set_run_font(r, size=11, color=MUTED)
    p = doc.add_paragraph()
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    ...
    add_table(
        ...
        [
            ("水管长度", "250 mm", "用户给定"),
            ("主动杆 / 蓝色连杆", "51.5 mm / 65.5 mm", "用户给定"),
            ("RS00轴心O高度", "38 mm", "用户给定"),
            ("水管合页轴C高度", "建议82 mm", "机构计算值，待装配验证"),
            ("对称软件硬限角", "暂定±8°", "几何安全值，待无球测试确认"),
            ("IMU频率", "500 Hz", "用户给定/部署约束"),
            ("外环控制频率", "200 Hz", "设计值"),
            ("视觉频率", "100 Hz", "用户最新确认"),
    ...
    add_body_paragraph(
        doc,
        "以水管固定端合页轴C为原点。O为RS00轴心，A为主动杆末端，B为蓝色连杆与水管的连接点。"
        "模型当前采用O=(232,−44) mm，即O在C右侧232 mm、低44 mm；水管水平时B位于C右侧250 mm。"
    )
    add_equation(doc, "A = O + r[cos(q + q₀), sin(q + q₀)]ᵀ")
    add_equation(doc, "B = C + L[cosθ, sinθ]ᵀ，且  ‖B − A‖ = l")
    add_body_paragraph(
        doc,
        "其中r=51.5 mm、l=65.5 mm、L=250 mm，q为RS00角度，θ为水管角度，q₀为装配零点偏置。"
        "每次控制先求期望水管角θ，再通过逆运动学求RS00目标角q。"
    )
    add_figure(
        doc,
        ROOT / "two_link_mechanism_analysis.png",
        "图2  两连杆机构仿真分析：−8°、0°、+8°构型，逆解、速度/力矩倍率和等效惯量随姿态变化。",
        width=6.3,
    )

    doc.add_heading("3.2 为什么建议C轴高度82 mm", level=2)
    add_equation(doc, "H_C = H_O + 44 mm = 38 mm + 44 mm = 82 mm")
    add_body_paragraph(
        doc,
        "真正决定连杆几何的是C轴和O轴的相对高度，不是支撑块外形高度。水管水平时，B相对O的"
        "水平偏差约18 mm、竖直偏差44 mm，因此OB≈47.5 mm，满足两圆相交条件14 mm<OB<117 mm。"
    )
    add_callout(
        doc,
        "加工基准",
        "机械图应优先标注“水管合页C轴中心距底板82 mm”。若水管外径为50 mm且台面直接托住"
        "水管底部，则台面约57 mm；若合页轴高出台面d mm，则实体台面高度为82−d mm。",
        fill=GREEN,
        color="2F6B3A",
    )
    add_body_paragraph(
        doc,
        "纯几何计算表明，从水平装配分支连续运动时，理论范围约为−10.3°至+16.9°。负方向在"
        "−10.3°附近接近折叠极限，因此取对称±8°作为暂定硬限位，给加工误差、间隙和冲击留出约2°余量。"
    )
    add_equation(doc, "14 mm ≤ ‖B(θ) − O‖ ≤ 117 mm")
    add_body_paragraph(
        doc,
        "在±8°范围内，RS00约需运动−17.2°至+14.9°；最不利速度/力矩换算倍率约2.51。"
        "82 mm高度与±8°限位是一组配套设计，任何轴高、水平距离或孔中心距变化都需要重新计算和标定。"
    )
    ...
    add_table(
        ...
        [
            ...
            ("车体俯仰/轴向加速度", "MSPM0 IMU", "500 Hz；默认4 ms", "状态预测与车辆前馈"),
            ("RS00角度/速度", "RS00 CAN", "500 Hz；约3.5 ms反馈延迟", "计算真实水管角及安全"),
    ...
    add_body_paragraph(
        ...
        "新相机帧到达时，只使用一次帧号，并按照拍摄时间戳进行延迟测量更新。"
    )
    add_equation(
        doc,
    ...
    add_body_paragraph(
        ...
        "±8°硬限角、1.2 rad/s斜率限制、积分抗饱和、两连杆逆解和RS00安全限制。"
    )
    add_callout(
        doc,
        "算法选择",
    ...
    add_table(
        ...
        ["估计误差|e|", "车辆策略", "水管策略"],
        [
            ("<5 mm", "按地图正常限速运行", "精密LQI，日常建议不超过±5°"),
            ("5–8 mm", "减小加速度和加加速度", "提高回中优先级"),
            ("8–10 mm", "停止继续加速，准备受控停车", "允许短时使用±7°"),
            ("≥10 mm", "暂停地图任务", "回中/救球模式，硬限位±8°"),
    ...
    add_body_paragraph(
        doc,
        "±8°能抵消的理想稳态轴向加速度约为g·tan8°≈1.38 m/s²，但还必须给位置反馈和车体俯仰留"
        "角度余量。因此正常地图规划建议先把车辆加减速度限制在0.6–0.8 m/s²，并限制加加速度；"
    ...
    add_body_paragraph(
        ...
        "固定步长为1/3000 s，使500 Hz、200 Hz和100 Hz任务落在公共仿真网格上。"
    )
    add_figure(
        doc,
    ...
    add_table(
        ...
        [
            ...
            ("IMU", "500 Hz、偏置、噪声和处理延迟", "sfun_vehicle_imu.m"),
    ...
    add_body_paragraph(
        doc,
        "标称仿真中，卡尔曼估计能够跟随钢球真实位置和速度，水管命令受±8°限制，机构保持有效，"
        "RS00力矩远低于硬件峰值。参考位置包含±50 mm阶跃，因此其目的主要是验证回中和跟踪能力。"
    )
    ...
    stress_rows = [
        ("低摩擦静止", "6.49", "25.00", "1.15", "0.58", "是", "否"),
        ("正常车辆运动", "6.42", "25.00", "1.18", "0.59", "是", "否"),
        ("启动与急刹", "6.57", "25.00", "1.18", "0.60", "是", "否"),
        ("坑洼+视觉退化", "6.89", "25.00", "1.61", "0.58", "是", "否"),
        ("最坏组合（优化）", "10.48", "28.25", "71.91", "0.60", "是", "否"),
        ("最坏组合（仅KF-LQI）", "13.17", "25.00", "52.73", "0.58", "是", "否"),
        ("3.0 m/s²超限脉冲", "22.68", "71.99", "272.92", "0.57", "否", "否"),
    ]
    add_table(
        doc,
        ["场景", "RMS/mm", "峰值/mm", "滑移\nmm/s", "力矩\nN·m", "机械安全", "严格1 cm"],
    ...
    add_callout(
        ...
        "前六项“机械安全”为真，只表示未掉球、连杆有效、端部余量和滑移满足原安全门槛。比赛报告应另行"
    ...
    add_body_paragraph(
        doc,
        "在相同最坏扰动下，加入车辆前馈和扰动估计后，RMS误差相对仅KF-LQI方案降低约20.4%。"
        "由于两套方案的端部保护触发时刻不同，本次优化方案峰值滑移反而较高，所以不能只按RMS选控制器，"
        "还要同时约束滑移和端部余量。3.0 m/s²脉冲产生71.99 mm峰值和272.92 mm/s滑移，说明该工况"
        "超出推荐物理包线。"
    )
    ...
    add_figure(
        ...
        "图6  相同200 Hz控制器和500 Hz IMU条件下，100、60与30 Hz视觉的位置误差对照。"
    ...
    add_table(
        ...
        [
            ("正常车辆运动", "100 Hz", "1.750", "5.000", "100%", "是"),
            ("正常车辆运动", "60 Hz", "1.585", "5.000", "100%", "是"),
            ("正常车辆运动", "30 Hz", "1.866", "5.000", "100%", "是"),
            ("启动、刹车与坑洼", "100 Hz", "2.721", "6.821", "100%", "是"),
            ("启动、刹车与坑洼", "60 Hz", "2.699", "7.222", "100%", "是"),
            ("启动、刹车与坑洼", "30 Hz", "2.709", "6.471", "100%", "是"),
        ],
    ...
    add_callout(
        doc,
        "结论",
        "100 Hz、60 Hz和30 Hz在本次两组固定随机场景中均通过±10 mm严格判据。100 Hz不在每个单次"
        "随机指标上都最小，原因包括噪声、丢帧相位、整帧延迟量化以及滤波器尚未针对各帧率重新整定；"
        "不能据此解释成较低帧率更优。100 Hz仍提供更密集的新测量和更小的时间量化上限，部署时必须"
        "携带曝光时间戳并使用OOSM历史回溯；任何帧率下仅做位置PID都不可靠。",
        fill=GREEN,
        color="2F6B3A",
    )
    ...
    add_table(
        ...
        [
            ...
            ("MSPM0 IMU接收", "500 Hz", "接收原始IMU或姿态/轴向加速度及采样时间戳"),
    ...
    validation_steps = [
        "机构标定：测量所有销轴中心距、C/O轴高度与水平距离；用电子水平仪记录−8°、−4°、0°、4°、8°时的RS00编码器角。",
        ...
        "台架闭环：先限制水管±2°和约0.8 N·m，再逐步开放到±5°、±8°。",
    ...
    conclusions = [
        ...
        "按O轴38 mm、主动杆51.5 mm、蓝杆65.5 mm和水管250 mm，建议C轴中心82 mm，暂定软件硬限角±8°。",
        "PSoC E84直接控制RS00并承担最终安全；MSPM0只负责500 Hz IMU采集，树莓派只提供视觉。",
        "当前压力测试可以发现机构和控制包线，但尚不能证明实物满足1 cm；摩擦、视觉延迟和RS00响应必须实测辨识。",
        "地图速度规划必须限制车辆加减速度和加加速度；正常起步建议先按0.6–0.8 m/s²以下设计，并由实车数据继续收紧。",
    ]
    ...
    params = [
        ...
        ("水管最大角", "±8°", "暂定硬限位"),
        ("水管角速度", "1.2 rad/s", "模型限值"),
        ("控制/IMU/视觉", "200 / 500 / 100 Hz", "任务设计"),
        ("视觉延迟", "25 ms处理 + 10 ms串口 = 35 ms", "模型假设"),
        ("IMU延迟", "4 ms", "模型假设"),
        ("RS00指令/反馈延迟", "4.5 / 3.5 ms", "模型假设"),
        ("LQI现有增益", "[3.328, 1.033, 0.9997]", "按40 mm尺度，非最终1 cm参数"),
        ("8 mm尺度候选增益", "[14.854, 2.086, 0.981]", "必须重新仿真和台架验证"),
    ]
    add_table(
        doc,
        ["参数", "当前值", "说明"],
