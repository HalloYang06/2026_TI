...
| 板卡 | 正式职责 | 建议频率 |
|---|---|---:|
| 天猛星 MSPM0G3507 | 红外循迹、底盘速度/差速环、IMU采集和带时间信息的底盘状态发布；不运行滚球算法 | 当前约29.1 Hz唯一IMU源样本；CAN可200 Hz保持发布，建议升级115200/200 Hz |
...
```mermaid
flowchart LR
    M["MSPM0G3507<br/>IMU、红外、底盘"] -->|"约30 Hz新IMU<br/>200 Hz CAN保持发布"| E["EdgeTalk M33/M55<br/>200 Hz shadow"]
...
- 上述三项来自较早Python基线。当前RS00两连杆Simulink已按新实测机构和约30 Hz唯一
  IMU源样本重跑：正常车辆工况在100 Hz视觉下RMS误差`4.199 mm`、峰值`8.701 mm`，
  严格1 cm通过；启停/制动/坑洼组合的峰值达`101.288 mm`，说明当前物理与控制包线
  尚不能覆盖该扰动。
- 可行组合恢复工况中，前馈+扰动估计在当前IMU下RMS为`17.78 mm`，仅KF-LQI为
  `33.26 mm`；115200 bit/s、200 Hz唯一IMU的what-if结果降至`10.62 mm`，但仍需实机验证。
