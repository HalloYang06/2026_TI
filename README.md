...
最终参赛作品已完整完成，并在 `2026-08-03` 合并完赛分支。正式测评现场出现电源供电
不稳定，系统未能按预期完成评分，最终 **未获奖**。仓库中的仿真、主机测试、实机计数和
代码提交用于证明各阶段的软件与控制工作，不代表正式测评通过，也不应被表述为获奖成果。

现有提交没有完整的现场母线电压、电流和欠压复位波形，因此赛后只把“供电不稳定”记录为
已知故障类别，不臆测唯一器件级根因。后续工程改进按以下顺序执行：

1. 对电池、DC/DC、逻辑电源和执行器母线同步记录最小电压、峰值电流与复位原因。
2. 分开评估控制/感知与电机动力域，检查共地、回流、线径、连接器和启动顺序。
3. 使用电机启动、堵转边界、多板同时启动等最坏负载做动态压降和热启动回归。
4. 在任务日志中加入 brown-out/欠压锁存与受控恢复，并建立测评前供电检查清单。

下文保留备赛各阶段的技术状态和验证边界，避免用赛后结论反向改写当时的真实证据。
...
| 板卡 | 正式职责 | 建议频率 |
|---|---|---:|
| 天猛星 MSPM0G3507 | 红外循迹、底盘速度/差速环、IMU采集和带时间信息的底盘状态发布；不运行滚球算法 | JY901S UART 115200；`0x51/0x52/0x53`源帧实测各约199.6 Hz；CAN镜像200 Hz |
...
```mermaid
flowchart LR
    M["MSPM0G3507<br/>IMU、红外、底盘"] -->|"accel/gyro/attitude<br/>源帧各约200 Hz"| E["EdgeTalk M33/M55<br/>200 Hz shadow"]
...
- 可行组合恢复工况中，前馈+扰动估计RMS为`10.62 mm`，仅KF-LQI为`13.46 mm`；
  200 Hz是当前仿真基线，实机三类源帧已验证为各约199.6 Hz。
