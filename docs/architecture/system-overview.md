...
滚球控制和底盘控制分层。MSPM0 保证循迹和底盘的本地实时性；当前树莓派只输出有采集时刻的视觉测量，NanoPi-M5作为单机备选；EdgeTalk 独占滚球状态估计与控制权。120 Hz 视觉更新不会阻塞 200 Hz LQG 和 1 kHz 安全门，板间通信中断也不会让旧数据被无限使用。
...
| 无刷编码器角度/速度 | RS00内部位置环与M33安全监督 | 让LQG输出的目标摆角真实落地，并检测跟随误差 |
...
| 图像采集与球心识别 | 120 Hz | 8.33 ms周期，单帧处理目标小于6 ms | 树莓派 |
...
首版执行器模式冻结为RS00的CSP位置模式：M55只输出摆杆目标角，M33完成`+-4 deg`限幅、`80 deg/s`限速、freshness和故障门，再以200 Hz写入受限`loc_ref`，同时设置保守的`limit_spd/limit_cur`。RS00内部闭合位置环和FOC，EdgeTalk不重复实现三相电流环。MIT位置-速度阻抗模式只作为CSP实测相位滞后无法满足包络时的备选；纯速度模式会累积角度漂移，纯力矩/电流模式对模型、摩擦和失联保护要求更高，均不作为首版。
...
flowchart TD
    ...
    V["120 Hz / 64 B球位置<br/>时间戳/置信度"] --> O
    I["200 Hz ax, ay, pitch, yaw rate"] --> O
    I --> F["非线性扰动前馈"]
    O --> L["离散LQR 200 Hz"]
    F --> S["目标摆角限幅/限速"]
    L --> S
    S --> G["M33安全门 1 kHz<br/>CAN目标 200 Hz"]
