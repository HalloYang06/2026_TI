...
| IMU DMA取样/解包 | 目标500 Hz，当前实测待定 | 0.5 ms | MSPM0 |
...
- 当前WIT到MSPM0的SysConfig是`9600 bit/s`，三类11字节帧的完整组理论上限约29.1 Hz，
  所以现有720 frame/s CAN流是200 Hz镜像，不是200 Hz新IMU采样。目标500 Hz完整组至少
  需要165 kbit/s，候选用`460800 bit/s`，但必须先确认具体WIT模块支持的波特率/输出率并
  用三类源计数实测。协议迁移见`shared/protocol/MSPM0_CAN_TELEMETRY_V2_PROPOSAL.md`。
