...
状态机与单独命令的后续差异还包括启动基准：LCD曾显示Q3 READY时，RS00参数读回为
1775 mrad，而标定水平是1721 mrad。旧状态机只检查RS00链路在线，SW1后先保持当前
1775 mrad，再由控制器突然切换到1721 mrad基准，给钢球注入额外初速度。Q3的
START_PENDING现在会以每次不超过20 mrad的小步先回到1721 mrad；位置误差不超过
3 mrad且速度不超过80 mrad/s连续100 ms后，才初始化估计器、清PID积分并启动5秒计时。
回平超过1.5秒则受控终止。

实机首次验证新启动流程时，Q3已经完成`+50 → -50`并置`passed=1`，但又暴露通用
15秒commission超时仍会把已通过的Q3切到phase4回水平，导致PID关闭后钢球从-50滑走。
当前已排除`mode=Q3 && phase=3 && passed=1`的通用超时：通过后持续闭环保持-50；
只有任务选择切换到Q4/Q5/Q6时才退出Q3并回水平。
