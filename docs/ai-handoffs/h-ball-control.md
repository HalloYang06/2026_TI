...
| RS00水管水平编码器角 | 1.7205 rad | 水平放置后只读采样1.720～1.721 rad |
...
## 2026-07-31 RS00水平零位与双向微动复核

- 操作者确认RS00/连杆架空无载、钢球取下、车轮离地或底盘动力断开、人在旁可立即断电。
- 水管机械放平后通过KitProg3 FinSH `COM20@115200`只读采样，`mechPos`显示范围
  `1.720～1.721 rad`，部署水平零位冻结为`1.7205 rad`。
- `hball_probe5`只读Get_ID成功；RS00母线约`22.5 V`、Iq约0，CAN TEC/REC、bus-off、
  FIFO full/lost和发送失败均为0。
- 人工台架状态机使用CSP、`0.5 rad/s`、`0.8 A`，先保持当前点再使能。正向测试
  `1.720 -> 1.730`目标，反馈到`1.731 rad`；回位后反馈约`1.724 rad`并自动stop。
- 负向测试从`1.724 -> 1.714`目标，反馈到`1.714 rad`；回位后反馈约`1.723 rad`
  并自动stop。两次fault均`0x00`，最终再次显式stop。
- 该结果只证明RS00编码器与CSP目标正负一致、双向10 mrad可执行。水管空间抬升方向仍需
  现场倾角观察确认；约1～4 mrad回位残差包含采样时刻、驱动停止和机构弹性，不能直接
  宣称为四杆回差。正式自动路径仍为`ACTUATOR_TX=0`。
- 放入钢球后执行单段`+20 mrad`：RS00从`1.712 -> 1.732 rad`且现场确认钢球向合页C
  （冻结位置负方向）运动。随后以`-10 mrad`回到`1.721 rad`并stop，fault=`0x00`。
  因此实机RS00编码器方向与几何四杆角方向相反，部署映射固定
  `HBALL_LINKAGE_MOTOR_DIRECTION_SIGN=-1`；该符号必须同时用于编码器正解和电机目标逆解。
...
- 后续实车确认该实现未经队友验证且起步异常，现已回退；当前恢复为
  `STBY`拉高后调用`motor_pwm_set()`，不得再把本节静态测试当作实车结论。
- `motor_start_synchronized()`在跑圈首次起步时保持H桥`STBY`为低，依次准备左右方向和
  PWM比较值，执行`__DSB()`后统一拉高`STBY`；运行中的速度环仍调用原
  `motor_pwm_set()`，不会周期性切断驱动。
- 静态测试验证调用顺序必须是`disable < preload both PWM < shared enable`，并保留实物
  已确认的左右轮独立方向写法，不使用整端口覆盖。MSP测试`20 passed`，Keil构建0 errors，
  `Code=40152, RO=15348, RW=144, ZI=6064`。
