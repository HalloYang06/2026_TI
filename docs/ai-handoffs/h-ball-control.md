...
增加直接角度编码器。当前WIT 9600 bit/s只能约29.1个完整源样本组/s，200 Hz CAN镜像
不能当成200 Hz新IMU样本。

MATLAB/Simulink R2025b于2026-07-31按新机构和约30 Hz唯一IMU源样本重跑：

- 100 Hz正常车辆场景：RMS `4.199 mm`、峰值`8.701 mm`，全程在`±10 mm`。
- 100 Hz启动/制动/坑洼：RMS `48.173 mm`、峰值`101.288 mm`，严格1 cm失败。
- 从`+25 mm`开始的可行组合恢复：优化方案RMS `17.782 mm`、峰值`45.729 mm`，
  仅KF-LQI为`33.260/88.038 mm`；两者机械安全，但这不是“全程1 cm”测试。
- 同一可行组合在115200/200 Hz唯一IMU what-if中为RMS `10.62 mm`、峰值`25.00 mm`；
  这是升级预测，不是硬件实测。
- 超新机构包线组合峰值`92.709 mm`、滑移`150.951 mm/s`，机械安全失败。
- 人为`3.0 m/s²`轴向超限脉冲：峰值`115.015 mm`、滑移`240.595 mm/s`，机械安全失败，
