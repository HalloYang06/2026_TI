...
- MSPM0 发布 `0x080/0x100/0x101/0x102/0x103` 五类 8 字节标准帧，
  频率分别为 `20/200/200/100/200 Hz`，合计`720 frame/s`。加速度、角速度和
  姿态的16位序号来自WIT `0x51/0x52/0x53`实际有效帧计数，重复CAN镜像保持
  相同序号；轮速和心跳使用各自发送序号。姿态与加速度/角速度同为200 Hz镜像，
  但镜像率不等于源采样率。
...
- JY901S UART为`115200 bit/s`。新固件10 s连续窗口中`0x51/0x52/0x53`
  各`+1996`，三类各约199.6 Hz，校验/未知帧增量为0。
...
跑圈起步使用`motor_start_synchronized()`：先保持双路H桥`STBY`为低，同时写好左右轮
方向和PWM比较值，执行数据同步屏障后再统一拉高`STBY`。该函数只替换跑圈首次启动，
运行中的左右轮PI和转向更新仍使用`motor_pwm_set()`，避免每周期反复切断驱动。
